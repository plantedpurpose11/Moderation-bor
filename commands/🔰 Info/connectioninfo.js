  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    let ls = client.settings.get(message.guild.id, "language")
      ...
      const embed = new Discord.MessageEmbed()
        ...
        .addField('➡️ **Channel**', `> **${member.voice.channel.name}** ${member.voice.channel}`, true)
        .addField('➡️ **Channel-ID**', `> \`${member.voice.channel.id}\``, true)
        .addField('➡️ **Members in there**', `> \`${member.voice.channel.members.size} total Members\``, true)
        .addField('➡️ **Full Channel?**', `> ${member.voice.channel.full ? "✅" : "❌"}`, true)
        .addField('➡️ **Bitrate**', `> ${member.voice.channel.bitrate}`, true)
        .addField('➡️ **User join limit**', `> \`${member.voice.channel.userLimit != 0 ? member.voice.channel.userLimit : "No limit!"}\``, true)
